...
ANS: The population distribution is right skewed and unimodal and there are outliers. 
...
ANS: The distribution of the 50 samples is very close to that of the population distribution.  The sample mean (point estimate) is close to the population mean.
...
ANS: The more the population, the standard error is less. there is a significant difference in mean between the samples. when the sample size is bigger, plot is more like the whole population. Based on the shape, standard error and mean, it looks like more accuracy in bigger size.
...
ANS:  5000 elements are selected in sample_mean50. The shape of the histogram is close to a normal distribution. mean is 1500 and same as whole population mean. the bigger the sample, the spread is small.
...
ANS: There are 100 elements or sample mean generated for each of the sample of sample size 50.
...
ANS: The center will be closer to the whole popilation and accurate. Spread is smaller.
...
```{r Y2, eval=TRUE, tidy = FALSE}
samplep_means_50<-rep(0,5000)
for( i in 1:5000){
sampp<-sample(price,50)
samplep_means_50[i]<-mean(sampp)
}
hist(samplep_means_50, breaks=25)  
summary(price)
```
