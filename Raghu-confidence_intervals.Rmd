...
ANS: The population distribution is right skewed and unimodal, as the median (1514) is less than the mean(1525). there is an extreme outlierover 5,000. the typical or the average living area in ames is 1525 sqft.

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

ANS: Another student's distribution will not be idential as the samples are random for each person. However, it would be somewhat similar because if the point esitmate or sample mean is unbiased then the sampling distribution of the estimate should be centered at the parameter it estimates.
...
ANS:The samples must be random and imdependent (less than 10% of the population), sample size greater than 30, and the disribution is not strongly skewed.

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.

ANS:95% confidence means that roughly 95% of the time the estimates will be within 2 standard errors of the parameter.
...
ANS: 95% confidence interval- 1373, 1677. mean(population) is 1500. mean is between the interval limit. Yes, neighbor's interval capture this value.
...
ANS: 95% of the intervals would capture the true population mean.
