...
mens heights histogram looks normal than female heights.
...
ANS: Its nearly normal distribution.
...
Answer: Not all of the points are on the line. comparing Simulation Vs heights data, it is similar on the 1st 2 standard deviations where it was mostly on the line, but the 3rd Sd go in opposite directions.
...
ANS: Plot for height does look similar compared to simulated data. it provides evidence that the female heights are nearly normal.
...
```{r sim-wgtnorm, eval=TRUE}
#create weight norm simulation
...
qqnorm(sim_wgtnorm)
qqline(sim_wgtnorm)
#compare to more plots
qqnormsim(fdims$wgt)
```

ANS: it does not seem to a normal distribution and appear to be opposite to the simulation..
...
Q1: What is the probability that a randomly chosen young adult female is shorter than 160 cm?”
```{r 6.q1, eval=TRUE}
#normal distribution
...
```  
Probability based on normal distribution is almost same as empirical distribution.


Q2: What is the probability that a randomly chosen young adult female is heavier than 60 Kg?
```{r 6.q2, eval=TRUE}
#normal distribution
