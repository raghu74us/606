...
We can use a scatterplot to display the relationship between runs and at_bats.
```{r 7.1, eval=TRUE}
q1 <- lm(mlb11$runs~mlb11$at_bats)
plot(mlb11$at_bats,mlb11$runs)
abline(q1)
```

The relationship is linear. Yes, the correlation coefficient is large enough that we can use the model to predict runs scored by a player from his at bats number.
...
There is a a linear relation between at_bats and ‘runs’ variable. It is a direct linear relationship that has a good correlation (~0.6). There are some non-significant outliers.
...
The smallest sum of squares that I got was from Run 3 - 130930.3. It this still larger than the sum of squares computed in exercise 2.
...
```
The manager will be over estimating the number of runs
...
7.  Based on the histogram and the normal probability plot, does the nearly 
 ...
 Yes it does. The points in the qqline are all within the cloud and no obvious outliers can be found.
...
Yes it appears to be met. The points don’t fan out of the normal line as would be the case if there were significant variability among the residual observations
