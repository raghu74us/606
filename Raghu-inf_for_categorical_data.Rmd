...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    sample) or *population parameters*?
     ...
     These data were taken from a poll so they are based on sample statistics. It would not be feasible to know the exact population parameters in this case.
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    assumption?
     ...
     The sample observations are independent. The sample size must be < 10% of the population and the sample      size must have been picked through random sampling within and across countries. We also expect to see       at least 10 successes and 10 failures in our sample. In this case, atheist/nonatheist, etc. sample size      should be 10 or greater.
...
3.  What does each row of Table 6 correspond to? What does each row of 
    `atheism` correspond to?
     ...
     Per country, % of religious person, not religious person, atheist and did not respond for the sample size. Response of a person in a given country and the year the poll was taken.
...
5.  Write out the conditions for inference to construct a 95% confidence
    ...
    Observations are independent. The poll is based on a simple random sample and consists of fewer than 10%     of the U.S. population, which confirm independence. Success-failure condition. The sample size of 1002     is large enough, as a success rate of 5% (atheism rate) is greater than 10. 5% of 1,002 is ~= 50.
...
```{r r6, eval=TRUE}
# margin of error = z-score for 95%
...
```{r canada-atheism, eval=TRUE}
#For Canada

ca12 <- subset(atheism, nationality == "Canada" & year == "2012")
ca12_atheist <- subset(atheism, nationality == "Canada" & year == "2012" & response =="atheist")
nrow(ca12_atheist)/nrow(ca12)

#Observations are independent. The poll is based on a simple random sample and consists of fewer than 10% of the Canada population, which verifies independence. Success-failure condition. The sample size of 1002 is large enough, as a success rate of 8% (atheism rate) is greater than 10. 8% of 1,002 is ~= 80.
...
pa12 <- subset(atheism, nationality == "Pakistan" & year == "2012")
pa12_atheist <- subset(atheism, nationality == "Pakistan" & year == "2012" & response =="atheist")
nrow(pa12_atheist)/nrow(pa12)

#Observations are independent. The poll is based on a simple random sample and consists of fewer than 10% of the Afghanistan population, which verifies independence. Success-failure condition. The sample size of 2704 is large enough, as a success rate of 2% (atheism rate) is greater than 10. 2% of 2704 is ~= 54.

inference(pa12$response, est = "proportion", type = "ci", method = "theoretical", 
    success = "atheist")
``` 
...
It is parabolic. ME increases with increasing p until it reaches a maximum at 0.5 (50%). Then ME decreases as p increases to 1 (100%)
...
```    
The larger the sample size n, the smaller the spread. The smaller the spread, the smaller the margin of error. 
...
11. If you refer to Table 6, you'll find that Australia has a sample 
    ...
    They both have a normal distribution with almost similar spreads, so their margin of errors should be       close. Ecaudor though violates the success-failure condition of 10 successful smaples since 0.02 of 400     is only 8.
* * *
## On your own
